//---------------------------------------------------------------------------------------------------------------------
// MPI_Scatter_Gather_Adv_Uneven.c
//
// Scatter and Gather test for unequal data distribution
//
...
int main(int argc, char* argv[])
{
	...
	int elementsPerProcess = BUFF_ELEMENTS / p; // e.g., BUFF_ELEMENTS = 14, p =3;
	MPI_Scatter(pSendBuffer, elementsPerProcess, MPI_INT, pReceiveBuffer, elementsPerProcess, MPI_INT, 0, MPI_COMM_WORLD);

	// Distribute the remaining data to the last node.
	int elementsPerProcessRemain = BUFF_ELEMENTS % p;
	if(my_rank == 0)
	{
		int offset = elementsPerProcess * p;
		MPI_Send((int*)pSendBuffer + offset, elementsPerProcessRemain, MPI_INT, (p-1), 99, MPI_COMM_WORLD);
	}
	else if(my_rank == (p-1))
	{
		MPI_Recv((int*)pReceiveBuffer + elementsPerProcess, elementsPerProcessRemain, MPI_INT, 0, 99, MPI_COMM_WORLD, &status);
	}
